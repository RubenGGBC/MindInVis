...
class PromptBuilder {
  ...
  static getNumberOfLLMItems(that) {
    if (!that || !that._styles) return ModelDefaultValues.NumberOfLLMItems.default;

    let numberOfItemsElement = that._styles.find((s) => s.name === 'Number of items');
    if (!numberOfItemsElement) return ModelDefaultValues.NumberOfLLMItems.default;

    if (ModelDefaultValues.NumberOfLLMItems.initial === numberOfItemsElement.value) {
      return ModelDefaultValues.NumberOfLLMItems.default;
    }
    return numberOfItemsElement.value;
  }

  static getNumberOfLogItems(that) {
    if (!that || !that._styles) return ModelDefaultValues.NumberOfLogItems.default;

    let numberOfItemsElement = that._styles.find((s) => s.name === 'Number of log items');
    if (!numberOfItemsElement) return ModelDefaultValues.NumberOfLogItems.default;

    if (ModelDefaultValues.NumberOfLogItems.initial === numberOfItemsElement.value) {
      return ModelDefaultValues.NumberOfLogItems.default;
    }
    return numberOfItemsElement.value;
  }

  static getDescription(that) {
    if (!that || !that._styles) return ModelDefaultValues.Description.default;

    let descriptionElement = that._styles.find((s) => s.name === 'Description');
    if (!descriptionElement) return ModelDefaultValues.Description.default;
  ...
  // Prompt principal para respuestas LLM
  ...
  // Prompt para respuestas basadas en PDF
  ...
  static getPromptForSummarizationAnswers(question, nodes, number) {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      if (i === 0) {
        prompt += 'ANSWERS= {\n' +
          '"node_name":' + (nodes[i]._info ? nodes[i]._info.title.replaceAll('\n', ' ') : '') + ',\n' +
  ...
  static getPromptForSummarizationQuestions(question, nodes, number) {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      if (i === 0) {
        prompt += 'QUESTIONS= {\n' +
          '"question_name":' + (nodes[i]._info ? nodes[i]._info.title.replaceAll('\n', ' ') : '') + ',\n' +
          '"description": ' + description + '\n' +
          '}';
      } else {
        prompt += ',{\n' +
    ...
    prompt += 'Summarization. I want you to behave as an academic. I have provided an ANSWER above and then a set of follow-up questions with descriptions in a JSON. Questions might not be fully alternative but some nuisance might exists among them. I want you to cluster the set of questions in ' + number + ' clusters that encloses those questions that are semantically closer.' + '\n';
    prompt += ' You have to provide the response in JSON format including each clustered item in an array. The format should be as follows:\n';
    prompt += '{\n' + '"clusters": [\n';
  ...
  // Sugerencia de pregunta basada en modelo
  static getPromptForModelSuggestedQuestion(that, answerNodeLabel, answerNodeNote, previousQuestionNodeLabel, firstQuestion, model) {
    let numberOfItems = model.numberOfQuestions || 3;
    let description = this.getDescription(that);
  ...
  // Sugerencia de preguntas basadas en logs
  static getPromptForLogsSuggestedQuestions(that, answerNodeLabel, answerNodeNote, previousQuestionNodeLabel, firstQuestion, logs) {
    ...
    if (logs && Array.isArray(logs)) {
      let logsData = logs.map(log => log.value || log);
  ...
  // Sugerencia de preguntas por LLM
